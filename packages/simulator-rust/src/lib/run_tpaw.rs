...
fn calculate_pre_withdrawal(
    ...
    // ------ RETURNS -----
    let expected_returns_legacy_portfolio = plan_params_processed
        .returns_stats_for_planning
        .stocks
        .empirical_monthly_non_log_expected_return
        * params.target_allocation.legacy_portfolio
        + plan_params_processed
            ...
            .bonds
            .empirical_monthly_non_log_expected_return
            * (1.0 - params.target_allocation.legacy_portfolio);
        ...
        let legacy = account.withdraw(
            (params.legacy_target * (1.0 + scale.legacy))
                / f64::powi(
                    ...
                    (plan_params.ages.simulation_months.num_months as usize - month_index) as i32,
                ),
        );
...
fn calculate_target_withdrawals(
    ...
    let lmp = plan_params_processed.by_month.risk.tpaw_and_spaw.lmp[month_index];
    let essential = plan_params_processed
        .by_month
        .adjustments_to_spending
        .extra_spending
        .essential
        .total[month_index];
    let discretionary = plan_params_processed
        .by_month
        .adjustments_to_spending
        .extra_spending
        .discretionary
