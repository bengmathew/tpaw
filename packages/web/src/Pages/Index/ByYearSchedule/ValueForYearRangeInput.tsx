  ...
  ({
    ...
    const [yearRange, setYearRange] = useState(() => {
      ...
      const numeric = (x: YearRangeEdge) =>
        _.clamp(numericYear({age}, x), age.start, age.end)
    ...
    const domain = _.range(params.age.start, params.age.end + 1).map(value => ({
      value,
      tick:
        value % 10 === 0
          ? ('large' as const)
          ...
          : ('none' as const),
      special:
        value === params.age.retirement && !isRetired ? '#DC2626' : undefined,
    }))
    const sliderProps = {
      className: '',
    ...
    const clampYear = (year: number) =>
      _.clamp(year, params.age.start, params.age.end)
              ...
              <div className=" p-2 w-[min(100vw-32px,400px)]">
                ...
                <div className="h-14 overflow-hidden  mt-1 text-sm p-1 grid gap-y-1">
                  ...
                  {fixesToDisplay.includes('lastWorkingYear') && (
                    <FixSwitch label="Fix to Last Working Year" fix="lastWorkingYear" />
                  )}
