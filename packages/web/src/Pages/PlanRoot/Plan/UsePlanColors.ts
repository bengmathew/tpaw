...
import { makeShades } from '../../../Utils/MakeShades'
import { ChartPointerOpts } from '../../Common/Chart/ChartComponent/ChartPointer'
import { useSimulation } from '../PlanRootHelpers/WithSimulation'

export const usePlanColors = () => {
  const { simulationInfoByMode } = useSimulation()

  return _planColors[`mainPlan`](simulationInfoByMode.mode)
}
...
export type PlanColors = {
  ...
  results: {
    ...
    rangeBand: string
    medianLine: string
    pointer: ChartPointerOpts['colors']
    minMaxYAxisFG: string
    menu: {
      bg: string
      currentBG: string
      chart: {
...
const _planColors = {
  mainPlan: (mode: 'history' | 'plan'): PlanColors => {
    ...
    return {
      ...
      results: block(() => {
        //745891
        const shades = makeShades(mode === 'plan' ? indigo[800] : indigo[950])
        const lightShades = makeShades(shades[3])
        const altShades = makeShades(makeShades(lightShades[5])[11])
        const fg = shades[14]
        const dark = shades[14]
        const fgForDarkBG = shades[2]
        const rangeShadeIndex = 6
        return {
          bg: altShades[6],
          cardBG: lightShades[1],
          fg: fg,
          darkBG: dark,
          fgForDarkBG,
          rangeBand: lightShades[rangeShadeIndex],
          medianLine: dark,
          print:{
            chartBG: altShades[1]
          },
          menu: {
            // bg: makeShades(lightShades[7])[11],
            bg: 'white',
            currentBG: altShades[10],
            chart: {
              // bg : lightShades[4]
              bg: altShades[4],
            },
          ...
          xAxis: {
            ...
            label: dark,
            bg: {
              retired: lightShades[4],
              notRetired: lightShades[2],
            },
            pointerBG: dark,
            tick: {
              large: dark,
