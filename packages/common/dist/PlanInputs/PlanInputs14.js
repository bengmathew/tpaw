...
exports.PlanInputs14 = void 0;
const lodash_1 = __importDefault(require("lodash"));
var PlanInputs14;
(function (PlanInputs14) {
    PlanInputs14.MAX_LABEL_LENGTH = 150;
    PlanInputs14.MAX_AGE = 120;
    PlanInputs14.MAX_NUM_YEARS_IN_GLIDE_PATH = 1000;
    const { number, string, constant, chain, object, union, array, boolean, intersection, } = Validator;
    ...
    const _intTest = (x) => {
        if (!lodash_1.default.isInteger(x))
            throw new Validator.Failed('Not an integer.');
        return x;
    };
    const _boundedStringTest = (bound) => (x) => {
        if (x.length > bound)
            throw new Validator.Failed('Too large.');
        return x;
    };
    const _trimmedTest = (x) => {
        if (x.trim().length !== x.length)
            throw new Validator.Failed('Not trimmed.');
        return x;
    };
    const _geTest = (y) => (x) => {
        if (x < y)
            throw new Validator.Failed(`Less than ${y}.`);
        return x;
    };
    const _leTest = (y) => (x) => {
        if (x > y)
            throw new Validator.Failed(`Greater than ${y}.`);
        return x;
    };
    // // ---------- FROM V13 ------------//
    // export const fromV13 = (v13: TPAWParamsV13.Params): Params => {
    //   const result: Params = {
    //     ...v13,
    //     v: 14,
    //     risk:
    ...
    //         ? {
    //             useTPAWPreset: false,
    //             tpawPreset: getDefaultParams().risk.tpawPreset,
    ...
    // }
    // ----------- VALIDATOR  ---------//
    const _ageRange = chain(number(), _geTest(0), _leTest(PlanInputs14.MAX_AGE));
    const _ages = chain(union(object({
        ...
        if (max < current + 1) {
            throw new Validator.Failed('Max age should be at least one year after current age.');
        }
        if (ages.type === 'notRetired') {
            const { retirement } = ages;
            if (retirement < current + 1) {
                throw new Validator.Failed('Retirement age should be at least one year after current age.');
            }
            if (max < retirement + 1) {
                throw new Validator.Failed('Max age should be at least one year after retirement age.');
            }
        }
        return ages;
    });
    const _person = object({
        ages: _ages,
        displayName: union(strBoundedTrimmed(PlanInputs14.MAX_LABEL_LENGTH), constant(null)),
    });
    ...
    const _valueForYearRange = object({
        ...
        label: union(strBounded(PlanInputs14.MAX_LABEL_LENGTH), constant(null)),
    ...
    const _glidePath = object({
        ...
        intermediate: sizedArray(object({ year: _year, stocks: numRangeInclusive(0, 1) }), 0, PlanInputs14.MAX_NUM_YEARS_IN_GLIDE_PATH - 2),
    ...
    PlanInputs14.validator = chain(object({
        v: constant(14),
        ...
        risk: intersection(union(object({
            ...
            tpawPreset: union(constant('riskLevel-1'), constant('riskLevel-2'), constant('riskLevel-3'), constant('riskLevel-4')),
            customTPAWPreset: union(_tpawRisk, constant(null)),
            savedTPAWPreset: union(_tpawRisk, constant(null)),
            swr: object({
        ...
        if (!x.risk.useTPAWPreset &&
            ...
            throw new Validator.Failed('Spending Floor is greater than spending ceiling.');
        }
            ...
            if (year.type === 'namedAge' || year.type === 'numericAge') {
                ...
                if (year.person === 'person1') {
                    ...
                    if (!x.people.withPartner) {
                        throw new Validator.Failed(`${prefix} is in terms of the age of an unspecified person.`);
                    }
                    person = x.people.person2;
                }
                if ((year.age === 'retirement' || year.age === 'lastWorkingYear') &&
                    person.ages.type === 'retired') {
                    throw new Validator.Failed(`${prefix} is in terms retirement age of ${year.person}, but ${year.person} is already retired.`);
                }
            }
        };
...
})(PlanInputs14 = exports.PlanInputs14 || (exports.PlanInputs14 = {}));
//# sourceMappingURL=PlanInputs14.js.map