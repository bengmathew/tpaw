...
import { PlanParams17 } from './Old/PlanParams17'

export namespace PlanParams18 {
  ...
  export const ADDITIONAL_SPENDING_TILT_VALUES = preciseRange(
    -0.05,
    0.05,
    0.001,
    3,
  ...
  const wealth = (params: Params | null): JSONGuard<Params['wealth']> =>
    object({
      ...
      futureSavings: valueForYearRangeArr(params?.people ?? null),
      retirementIncome: valueForYearRangeArr(params?.people ?? null),
    })
  ...
  const risk = (params: Params | null): JSONGuard<Params['risk']> =>
    object({
      tpaw: object({
        ...
        additionalSpendingTilt: among(ADDITIONAL_SPENDING_TILT_VALUES),
      }),
      tpawAndSPAW: object({
  ...
  export const guard: JSONGuard<Params> = (x: unknown) => {
    ...
    if ((x as any).v === 18) {
      ...
      const prevResult = PlanParams17.guard(x)
