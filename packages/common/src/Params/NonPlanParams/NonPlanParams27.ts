...
import { NonPlanParams26 as NonPlanParamsPrev } from './Old/NonPlanParams26'
export namespace NonPlanParams27 {
  ...
  export type NonPlanParams = {
    ...
    numOfSimulationForMonteCarloSampling: 'default' | number
  ...
  export const guard: JSONGuard<NonPlanParams> = object({
    ...
    showOfflinePlansMenuSection: boolean,
    numOfSimulationForMonteCarloSampling: union(
      constant('default'),
      chain(number, integer, gt(0)),
    ),
  ...
  export const migrate = (x: SomeNonPlanParams): NonPlanParams => {
    ...
    const result: NonPlanParams = {
      ...
      numOfSimulationForMonteCarloSampling:
        prev.numOfSimulationForMonteCarloSampling === 500
          ? 'default'
