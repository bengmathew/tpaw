...
import { PlanParams25 as PlanParamsPrev } from './Old/PlanParams25'

export namespace PlanParams26 {
  export const currentVersion = 26 as const
  ...
  export const migrate = (x: SomePlanParams): PlanParams => {
    ...
    const result: PlanParams = {
      ...
      advanced: {
        ...prev.advanced,
        expectedAnnualReturnForPlanning: block(() => {
          const p = prev.advanced.expectedAnnualReturnForPlanning
          switch (p.type) {
            case 'suggested':
              return { type: 'conservativeEstimate,20YearTIPSYield' }
            case 'oneOverCAPE':
              return { type: '1/CAPE,20YearTIPSYield' }
            case 'regressionPrediction':
              return { type: 'regressionPrediction,20YearTIPSYield' }
            default:
              return p
          }
        }),
