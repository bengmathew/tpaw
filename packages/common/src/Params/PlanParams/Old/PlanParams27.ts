...
export namespace PlanParams27 {
  ...
  export const MANUAL_STOCKS_BONDS_NON_LOG_ANNUAL_RETURNS_VALUES = preciseRange(
    -0.01,
    0.1,
    0.001,
    3,
  ...
  export const componentGuards = (() => {
    ...
    > = union(
      ...
      object({
        ...
        stocks: among(MANUAL_STOCKS_BONDS_NON_LOG_ANNUAL_RETURNS_VALUES),
        bonds: among(MANUAL_STOCKS_BONDS_NON_LOG_ANNUAL_RETURNS_VALUES),
      }),
    )
    ...
    > = object({
      standardDeviation: object({
        stocks: object({ scale: among(STOCK_VOLATILITY_SCALE_VALUES) }),
  ...
  export const migrate = (x: SomePlanParams): PlanParams => {
    ...
    const result: PlanParams = {
      ...
      advanced: {
        ...
        sampling: {
          type: prev.advanced.sampling.type,
          forMonteCarlo: {
            blockSize: prev.advanced.sampling.blockSizeForMonteCarloSampling,
            staggerRunStarts: true,
          },
        },
        expectedReturnsForPlanning:
          prev.advanced.expectedAnnualReturnForPlanning,
        historicalMonthlyLogReturnsAdjustment: block(() => {
          const p = prev.advanced.historicalReturnsAdjustment
          return {
            standardDeviation: {
              stocks: { scale: p.stocks.volatilityScale },
              bonds: { enableVolatility: p.bonds.enableVolatility },
            },
